...
**仓库角色**：本仓库为 **PC 端 Web 模板**（React 18 + Ant Design 5），用于快速搭建中后台或桌面优先类产品。

**分支说明**：本分支为 `react18antd5`。其他变体见仓库 `README.md`「其他分支 /多分支开发」；各分支的 `AGENTS.md` 与 `.cursor/rules` **独立维护**，不强制与其他分支保持一致。

## 工程概览

- **技术栈**：React 18、TypeScript、Vite 7、React Router 7（`HashRouter`）、**Ant Design 5**、Less、Zustand、ahooks；通知与反馈优先使用 **Ant Design** 组件能力（如 `message`/`notification`，按项目现有封装为准）。
- **运行要求**：Node.js **>= 20**（见 `package.json` 的 `engines`）。
- **路径别名**：`@/` → `src/`。
- **路由**：路由表在 `src/router/routes.tsx`；全局守卫在 `src/router/AuthGuard.tsx`。组件内跳转使用 `useNavigate()`。
