...
- **技术栈**：React 19、TypeScript、Vite 7、React Router 7（`HashRouter`）、**Ant Design 6**、**TailwindCSS v4**（优先）、Less（补充）、Zustand、ahooks；通知与反馈优先使用 **Ant Design** 组件能力（如 `App`/`message`/`notification`，按项目现有封装为准）。
